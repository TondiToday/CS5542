...
object decisiontree{
  ...
  def generateDecisionTreeModel(sc: SparkContext): Unit = {
    if (Files.exists(Paths.get(IPSettings.DecisionTree))) {
      println(s"${IPSettings.DecisionTree} exists, skipping DecisionTree model formation..")
      return
    }
    ...
    // Save and load model
    randomForestModel.save(sc, IPSettings.DecisionTree)
    println("DecisionTree Model generated")
  }
  ...
  def testImageClassification(sc: SparkContext) = {
    ...
    val nbModel = DecisionTreeModel.load(sc, IPSettings.DecisionTree)
  ...
  def main(args: Array[String]) {
    ...
    System.setProperty("hadoop.home.dir","C:\\Users\\tondi\\Dropbox\\COMP SCI 5542\\winutils")
