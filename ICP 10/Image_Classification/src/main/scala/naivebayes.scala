...
object naivebayes {
  ...
  def generateRandomForestModel(sc: SparkContext): Unit = {
    if (Files.exists(Paths.get(IPSettings.Naive_Bayes))) {
      println(s"${IPSettings.Naive_Bayes} exists, skipping Naive Bayes model formation..")
      return
    }
    ...
    // Save and load model
    model.save(sc, IPSettings.Naive_Bayes)
  ...
  def testImageClassification(sc: SparkContext) = {
    ...
    val nbModel = NaiveBayesModel.load(sc, IPSettings.Naive_Bayes)
